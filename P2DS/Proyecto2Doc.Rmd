...
Regresión Lineal: Es la mejor manera de describir la relación entre dos variables. Su objetivo es dibujar un valor que no se puede estimar inicialmente. Es parte del aprendizaje supervisado. El objetivo de la regresión lineal es encontrar la ecuación que se ajuste a la mejor línea para modelar los datos.

Clustering: También conocido como análisis de conglomerados, consiste en agrupar cada punto del conjunto de datos en un grupo específico. Los datos agrupados comparten una similitud entre sí. La agrupación es un ejemplo de método de aprendizaje no supervisado.

Q-learning: Es el método más popular de aprendizaje por refuerzo. Le permite establecer una política de acción de forma interactiva y mostrar qué comportamiento tomar bajo ciertas circunstancias.

Series de Tiempo Temporales: Una serie temporal es un conjunto de muestras tomadas a intervalos de tiempo regulares. Es interesante analizar su comportamiento al mediano y largo plazo, intentando detectar patrones y poder hacer pronósticos de cómo será su comportamiento futuro. Lo que hace especial a una Time Series a diferencia de un problema de Regresión son dos cosas:
...
Se tiene que en el sector de gCC hay 84 productos con venta total igual a 0
```{r}
...
* Paquete de limpieza docena de 14 (Jabón de manos Soft Blossom) con 0 venta.
* Top Family Rosa MOSQ Crema Corp 1KG Edi. Mama OFE 2, con 0 venta.
* Paquete Bazar 5, con 0 venta. 
```{r}
tail(gCC,3)
```
...
Se tiene que en el sector de g0021 hay 51 productos con venta total igual a 0
```{r}
...
* SC Nat Te Piernas Cansad Caj 20 BO OFE, con 0 venta.
* Acne Solution Mascar EXF C/Soya 10, con 0 venta.
* Top Family Jabón Liquido Neutro Para Zonas Intimas 800ML OFE 2, con 0 venta. 
```{r}
tail(g0021,3)
```
...
Se tiene que en el sector de g0005 hay 37 productos con venta total igual a 0
```{r}
...
* SC Naturals Te Descanso Caja 20 Unidades OFE, con 0 venta.
* SC Naturals Te Limp Intes Caj con 20 bol OFE, con 0 venta.
* SC Nat Te Piernas Cansad Caj 20 BO OFE, con 0 venta. 
```{r}
tail(g0005,3)
```
...
Se tiene que en el sector de g0027 hay 46 productos con venta total igual a 0
```{r}
...
Se tiene que en el sector de g0006 hay 49 productos con venta total igual a 0
```{r}
...
* Acne Solution Gel Trat C/Almend 12, con 0 venta.
* Acne Solution Mascar EXF C/Soya 10, con 0 venta.
* Action Locion Capilar Anticaida 180 ML, con 0 venta. 
```{r}
tail(g0006,3)
```
...
Se tiene que en el sector de gPR01 hay 4 productos con venta total igual a 0
```{r}
...
Se tiene que en el sector de g0038 hay 70 productos con venta total igual a 0
```{r}
