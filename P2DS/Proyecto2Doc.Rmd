...
## Se crea una lista de las iniciales de los sectores que se quieren eliminar

### Si el sector es un número, se debe de poner el número completo

noQuieroSecGuate <- c("AB", "0000")

## Se crea una lista de las iniciales de las categorías que quiero eliminar (Aquí puede ser el nombre completo)

noQuieroCatGuate <- c("KIMBERLY", "DESCONTINUADOS", "	CUIDADO DE LAS UÑAS")

## Se crea la función para lo que se quiere eliminar

noDeseado <- function(pais, noSec, noCat){
  for (i in noSec) {
    pais <- pais[-grep(i, pais$Sector)]
  }
  for (i in noCat){
    pais <- pais[-grep(i, pais$Sector)]
  }
...
SecG <- noDeseado(SecG, noQuieroSecGuate, noQuieroCatGuate)
