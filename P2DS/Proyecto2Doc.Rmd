...
setwd("C:/Users/Home/Documents/Tercer año/Sexto Semestre/Data Science/Proyecto2DS/P2DS")
...
En esta parte, se creó una función que eliminara las primeras dos filas de todo el data set, ya que estas contenían el nombre de las columnas. Asimismo, se decidió dejar solamente cuatro columnaas: Sector, Categoría, Producto y Ventas Finales. Todo lo demás se eliminó y se dejó solamente la información de todos los productos de estas cuatro categorías. 
...
eliminarNegativosCeros <- function(x){
  x <- x[x$VentasTotales >= 0, ]
}
...
# 
categoriasH <- data.frame(SecH$Categoria)
 n_occurH <- data.frame(table(categoriasH$SecH.Categoria))
n_occurH[n_occurH$Freq > 1,]
categoriasH[categoriasH$SecH.Categoria %in% n_occurH$Var1[n_occurH$Freq > 1],]
n_occurH$Freq <- NULL
listaCatH<- t(n_occurH[-c(1),])
listaCatH <- gsub(" ","", listaCatH)
listaCatH <- sort(listaCatH)
...
## Productos que más y menos venden según los sectores que más venden
